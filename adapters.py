...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts):
        pos_emb = self.positional_embedding.to(prompts.device).type(self.dtype)
...
def _get_base_text_features(cfg, classnames, clip_model, text_encoder, pretrained_projection=None):
    ...
    # dataset が CUSTOM_TEMPLATES にあればそれを使い、無ければデフォルト1種を追加
    if dataset in CUSTOM_TEMPLATES:
        TEMPLATES += [CUSTOM_TEMPLATES[dataset]]
    else:
        # CIFAR10など何も定義されてないケース
        TEMPLATES += ["a traffic sign that says '{}'","a photo of a {} sign"]  # 汎用のテンプレにしておく
...
class AdapterMethod(nn.Module):
    def __init__(self, cfg, clip_model, base_text_features):
        ...
        if self.initialization == "RANDOM":  # Randomly initialized Linear Probing
            ...
            self.init_tipA()
        elif "TipF" in self.initialization:
            self.init_tipA()
        elif "CrossModalA" in self.initialization:
            print("Using CrossModalA for Linear Probing", end="\n")
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        self.clip_model = clip_model  # ★ココで保持する
...
class ADAPTER(TrainerXCostume):
    ...
    def parse_batch_test(self, batch):
        """
        Dassl標準の parse_batch_test は batch["img"], batch["label"] を期待するが、
        今回の dataset は (img_tensor, label_tensor) 形式を返すので、オーバーライドする。
        """
        input, label = batch  # batch is a tuple/list of (img_tensor, label_tensor)
        input = input.to(self.device)
        label = label.to(self.device)
        return input, label
    def build_data_loader(self):
        """
        オーバーライドして、デフォルトの DataManager(build_dataset) を呼ばないようにする。
        すでに train_v12_total.py 側で独自の DataLoader (train_loader_x, val_loader, test_loader)
        を用意しているため、それらを使う前提の実装にする。
        """
        print(">> [INFO] Overriding build_data_loader in ADAPTER to skip DataManager call.")
        pass
    ...
    def build_model(self):
        ...
        # ここでは trainer.classnames を参照 (train_v12_total.pyでセット)
        SELECTED_LABELS = [
            "Speed Limit 30 km/h",
            "Main road",
            "Yield",
        ...
        classnames = SELECTED_LABELS
        if not classnames:
            raise ValueError("self.classnames is empty in build_model! Please set trainer.classnames = [...].")

        if not classnames:
            print("[WARN] self.classnames is empty. Did you forget to set trainer.classnames = [...]?")
        print(f"Loading CLIP (backbone: {cfg.MODEL.BACKBONE.NAME})")
        
        # ★ ここで lab2cname を作る (label_id -> クラス名)
        self.lab2cname = {i: cname for i, cname in enumerate(classnames)}



        print(f"Loading CLIP (backbone: {cfg.MODEL.BACKBONE.NAME})")
        clip_model = load_clip_to_cpu(cfg)
        # GPU利用なら clip_model を GPU に移す
