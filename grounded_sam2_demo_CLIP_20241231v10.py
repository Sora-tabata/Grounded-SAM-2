...
class CLIPRelabeler:
    ...
    def _normalize_similarities(self, similarity_matrix):
        ...
        # 2) 重み付け (ここでは freq_factor を使わず length_factor のみ)
        weights = torch.ones_like(normalized_similarity[0])
        for i, class_name in enumerate(self.class_names):
            # クラス名の単語数による単純な補正
            length_factor = 1.0 / (1.0 + len(class_name.split()))
            # freq_factor を無効化（常に 0.0 として扱う or 1.0 として扱う など）
            # freq = self.class_stats['counts'].get(class_name, 0)
            # freq_factor = 0.0  # ここで無効化
            weights[i] = length_factor  # freq_factor 加算せずに length_factor のみ
