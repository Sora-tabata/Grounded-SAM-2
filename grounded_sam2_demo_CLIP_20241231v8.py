...
class TipAdapterCLIP(torch.nn.Module):
    ...
    def __init__(self, base_clip_name="ViT-B/32", device="cuda"):
        ...
        # Adapter の部分を後で読み込む想定。ここでは空の nn.ModuleDict 等を作成しておく
        # （Tip-Adapter の実装によって異なります）
    ...
    def load_tip_adapter(self, tip_adapter_checkpoint):
        ...
        # 必要に応じてキー名の除外や変更を行いながら読み込む:
        # 例: self.load_state_dict(state_dict, strict=False)
    ...
    def encode_image(self, images):
        ...
        # ここで Adapter を通す (実装次第)
        # 例: base_feat = self.adapter_module(base_feat)
        # 以下はダミー: 何もしない例
        return base_feat
...
def process_image(args):
    ...
    # --- 修正 ---
    # Tip-Adapter 用のチェックポイントを読み込む CLIPRelabeler
