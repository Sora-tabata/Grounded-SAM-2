...
output_folder = '/mnt/source/Grounded-Segment-Anything/output_folder/'
        ...
        if file == 'grounded_sam2_results.json':
            ...
            with open(os.path.join(root, file), 'r') as f:
                ...
                for entry in mask_data:
                    if 'box' in entry:
                        updated_labels = update_mask_labels(entry['label'], labels)
...
def calculate_metrics_per_class(gt_dict, pred_dict, iou_threshold=0.8):
    ...
    for image_id in gt_dict:
        ...
        pred_labels = pred_dict[image_id]
        for gt in gt_labels:
            gt_label = gt['label']
            gt_bbox = gt['bbox']
            found_match = False
        ...
        for pred in pred_labels:
            pred_label = pred['label']
            if pred_label not in [gt['label'] for gt in gt_labels]:
    ...
    for label, metrics in class_metrics.items():
        ...
        ap_per_class[label] = precision
        iou_per_class[label] = tp / (tp + fp + fn) if tp + fp + fn > 0 else 0
    
    return ap_per_class, iou_per_class
